...
public class SqlBuilder extends BaseBuilder {
    ...
    private void selectSqlBuilder( BiReportBuildInDTO biReportBuildInDTO ) {
        ...
        if (Objects.nonNull(dimensionConditionBeanList) && !dimensionConditionBeanList.isEmpty()) {
            dimensionConditionBeanList.forEach(dimension -> {
                ...
                //拼接查询项
                String fieldAlias = selectBuilder(dimension.getFieldName(), dimension.getDataType(), FUNC_GROUP.getCode(),
                        dimension.getFieldFormula(), dimension.getIsBuildAggregated(), dimension.getGranularity(), false);
                //收集字段名与中文名映射
                fieldMapBuilder(fieldAlias, dimension.getFieldDescription(), dimension.getAliasName());
        ...
        if (Objects.nonNull(compareConditionList) && !compareConditionList.isEmpty()) {
            compareConditionList.forEach(compare -> {
                ...
                //拼接查询项
                String fieldAlias = selectBuilder(compare.getFieldName(), compare.getDataType(), FUNC_COMPARE.getCode(),
                        compare.getFieldFormula(), compare.getIsBuildAggregated(), compare.getGranularity(), false);
                //收集字段名与中文名映射
                fieldMapBuilder(fieldAlias, compare.getFieldDescription(), compare.getAliasName());
        ...
        indexConditionBeanList.forEach(index -> {
            ...
            //查询项SQL拼接
            String fieldAlias = selectBuilder(index.getFieldName(), index.getDataType(), index.getAggregator(),
                    index.getFieldFormula(), index.getIsBuildAggregated(), index.getGranularity(), qoqFlag);
            //指标项收集
            indexList.add(fieldAlias);
            //构造字段名与中文名映射集合
            fieldMapBuilder(fieldAlias, index.getFieldDescription(), index.getAliasName());
        });
    }
     ...
     * @return 字段别名
     */
    private String selectBuilder( String fieldName, String dataType, String aggregatorType, String formula,
            ...
            filterConditionBeanList.forEach(filter -> {
                ...
                if (filter.getIsBuildAggregated() == 0) {
                    ...
                    if (!Strings.isNullOrEmpty(whereStr)) {
                        ...
                        //同环比不受日期筛选条件的限制
                        if (!Objects.equals(filter.getDataType(), Constant.DataFieldType.DATETIME_TYPE.getType())) {
    ...
    private void orderBySqlBuilder( BiReportBuildInDTO biReportBuildInDTO ) {
        ...
        if (Objects.nonNull(sortList) && !sortList.isEmpty()) {
            sortList.forEach(sort -> orderByBuilder(sort.getFieldName(), sort.getSortFlag(), sort.getSortType()));
        }
    }
     ...
     */
    private void orderByBuilder( String fieldName, String sortFlag, int sortType ) {
        String sort = sortFlag;
        if (sortType == 0) {
            if (Strings.isNullOrEmpty(sort)) {
                sort = Constant.SORT_ASC;
            }
            fieldName = fieldAndAliasMap.get(fieldName);
    ...
    private void sparkAggBuilder( List<IndexConditionBean> indexList ) {
        ...
        indexList.forEach(index -> {
            String fieldAliasName = fieldAndAliasMap.get(index.getFieldName());
        ...
        if (Objects.nonNull(indexCondition)) {
            ...
            for (IndexConditionBean index : indexCondition) {
                if (index.getQoqType() > 0) {
                    ...
                    String fieldName = qoq.getFieldName();
                    String fieldAliasName = fieldName;

                    //生成别名
                    fieldAliasName = fieldAlias(FUNC_QOQ.getCode(), fieldName);
                    if (!DATE_WEEK.getCode().equals(qoq.getGranularity())) {
                        //对应日期格式化
                        fieldName = dateFieldFormat(fieldName, DataFieldType.DATETIME_TYPE.getType(), qoq.getGranularity());
                        //添加到查询项
                        selectQoqSqlList.add(String.format(" %s as `%s` ", fieldName, fieldAliasName));
                        //添加到where条件中
                        whereList.add(String.format(" %s IN ('%s', '%s')", fieldName, qoq.getQoqRadixTime(), qoq.getQoqReducedTime()));
                    } else {
                        ...
                        String where = String.format("DATE_FORMAT(%s, 'y') || '-' || WEEKOFYEAR(%s) in (%s)", fieldName, fieldName, inWhere);

                        //添加到查询项
                        selectQoqSqlList.add(String.format(" DATE_FORMAT(%s, 'y') || '-' || WEEKOFYEAR(%s) AS `%s` ", fieldName, fieldName, fieldAliasName));
                        //添加到where条件中
                        whereList.add(where);
                    }

                    qoq.setDelQoqField(true);
                    qoq.setQoqTimeAliasName(fieldAliasName);
                    qoq.setQoqIndexAliasName(fieldAndAliasMap.get(index.getFieldName()));
                    qoqList.add(qoq);
                    //分组字段不为空，将筛选字段添加到分组中
                    if (!groupSqlList.contains(fieldAliasName)) {
                        groupQoqSqlList.addAll(groupSqlList);
                        groupQoqSqlList.add(fieldAliasName);
                    }
                }
            }
